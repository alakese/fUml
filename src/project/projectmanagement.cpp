...
#include "project.h"
#include "../gui/dialogs/projectinputdialog.h"
...
ProjectManagement::ProjectManagement()
{
    ...
    if (db.isValid())
        ...
        MonitorManager::getInstance()->logMsg("INFO : sql driver connection successfull", 0);
    else
        /* Send the info to monitor */
        MonitorManager::getInstance()->logMsg("ERROR : DB driver not valid or can not be loaded", 0);
...
//    {
//        delete m_listProjects.at(i);
//    }
    // qDeleteAll (m_listProjects.begin(), m_listProjects.end());
}

/*!
 * \brief This function creates a new project and adds it to the list of the projects which will be shown e.g. in tree widget. */
bool ProjectManagement::createNewProject(GraphicsScene *scene)
{
    bool ret = false;
...
void ProjectManagement::setActiveProject(const QString &newActiveProject)
{
    ...
    foreach(Project *p, m_listProjects)
    {
        ...
        if (projName == newName)
        {
            ...
            MonitorManager::getInstance()->logMsg("INFO : Active project changed to %0", 1, newActiveProject.toLocal8Bit().data());
            /* Sent the signal to MainWindow::updateGUIForActiveProject() */
            emit activeProjectChanged(m_pActiveProject);
            return;
        }
    }
}
