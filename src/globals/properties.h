...
public:
    ...
    QString string(bool onlyNames)
    {
        ...
        /* If there is a stereotype and only names chosen */
        if (!stereotype.isEmpty() && !onlyNames)
        {
            QString str = QString(" <<%1>>").arg(stereotype);
            prefix.append(str);
        }
        ...
        if (onlyNames)
        {
            memName = QString("%1 %2").arg(prefix).arg(name);
        }
        else 
        {
            if (!initValue.isEmpty())
                memName = QString("%1 %2:%3=%4").arg(prefix).arg(name).arg(type).arg(initValue);
            else
                memName = QString("%1 %2:%3").arg(prefix).arg(name).arg(type);
        }
...
public:
    ...
    {  
        ...
        QString funcName = QString("%1 %2(").arg(prefix).arg(name);
        /* If only names chosen */
        if (onlyNames)
        {
            funcName.append(")");
        }
