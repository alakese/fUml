...
{
    return QRectF(0, 0, m_rectHeader.width(), 
                  m_rectHeader.height() + m_rectMembers.height() + m_rectFunctions.height());
}
...
void ClassItem::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    ...
    /* R-Click menu items*/
    QMenu menu;
    QAction *p_actRenameItem = menu.addAction(QIcon(), tr("Rename item..."));
    connect(p_actRenameItem, &QAction::triggered, this, &ClassItem::changeItemProperty);
    QAction *p_actDeleteItem = menu.addAction(QIcon(), tr("Delete item"));
    connect(p_actDeleteItem, &QAction::triggered, this, &ClassItem::deleteClassItem);
    menu.addSeparator();
    QAction *p_actChangeFont = menu.addAction(QIcon(), tr("Change font..."));
...
//        QUndoCommand *command = new ChangeFontCommand(this);
...
void ClassItem::drawMembers(QPainter *painter, int *posHeightOfTextLines, qint8 fontSizeInScene)
{
    ...
    if (m_listMemberProperties.isEmpty())
    {
        /* If members empty, then we need the min height : which is one item height */
        membersCount = 1;
    }
    else
    {
        ...
        if (m_bSplitTheString)
        {
            ...
            foreach(MemberProperties *member, m_listMemberProperties)
            {
                ...
                QStringList memberSplitted = splitItemString(member->string());
...
void ClassItem::drawFunctions(QPainter *painter, int *posHeightOfTextLines, qint8 fontSizeInScene)
{
    ...
    if (m_listFunctionProperties.isEmpty())
    {
    ...
    else
    {
        ...
        if (m_bSplitTheString)
        {
            ...
            {
                /* Calculate the size with splitted texts */
                QStringList functionSplitted = splitItemString(function->string());
...
void ClassItem::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (pos() != m_ptOldPosition)
        // emit itemMoved(this, m_ptOldPosition.toPoint());
        emit itemMoved(this);
    else
    {
...
/*!
 * \brief This function renames the class name. */
...
//    ClassItemDialog dlg(m_pGeneralProperties, m_listMemberProperties, m_listFunctionProperties, currentPage);
...
int ClassItem::calculateMinWidth()
{
    ...
    foreach(MemberProperties *member, m_listMemberProperties)
    {
        int w = fontMetrics.width(member->string());
        if (w > width)
            width = w;
    }
    /* Check functions */
    foreach(FunctionProperties *function, m_listFunctionProperties)
    {
        int w = fontMetrics.width(function->string());
        if (w > width)
            width = w;
    }
